...
class VAE(nn.Module):
    ...
    def load(path):
        ...
        vae = VAE(**config)
        vae.load_state_dict(torch.load(path, map_location=torch.device('mps')))
        return vae
...
if __name__ == "__main__":
    ...
    if args.load_disc:
        discriminator.load_state_dict(torch.load(args.load_disc, map_location=torch.device('mps')))
